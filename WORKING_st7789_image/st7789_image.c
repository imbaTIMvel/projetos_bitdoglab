...
void st7789_draw_bitmap(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint8_t *data) {
    if ((x >= ST7789_WIDTH) || (y >= ST7789_HEIGHT)) return;
    if ((x + w - 1) >= ST7789_WIDTH)  w = ST7789_WIDTH - x;
    if ((y + h - 1) >= ST7789_HEIGHT) h = ST7789_HEIGHT - y;
...
int main() {
    ...
    sleep_ms(100);
    // Draw sans
    st7789_draw_bitmap(0, 0, SANS_WIDTH, SANS_HEIGHT, sans);
    sleep_ms(1000);
    // Fill black
    st7789_fill_screen(RGB565(0,0,0));
    sleep_ms(100);
    // Draw FEEC logo
    st7789_draw_bitmap(0, 0, FEEC_WIDTH, FEEC_HEIGHT, feec);
    sleep_ms(1000);
